...
void draw_days()
{
   ...
   for (int i = 0; i < n; i++)
   {
      ...
      if (years[GlobY].months[GlobM].days[i].is_empty != 1) //выделение заметок
         putimage(PLANS_OFFSET_X+dx, PLANS_OFFSET_Y+dy, images[IMG_EDAY], 0);


      /* if(p == today)  //выделение текущего дня
           {
            GlobD = 1;
...
// Проверить, нажаты ли дни календаря и если да выставить GlobD нажатым днем
